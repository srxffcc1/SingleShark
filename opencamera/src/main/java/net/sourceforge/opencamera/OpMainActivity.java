 ...
 */
public class OpMainActivity extends Activity implements AudioListener.AudioListenerCallback {
	...
	private OpApplicationInterface applicationInterface;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.opencamera_activity_main);
		...
		applicationInterface = new OpApplicationInterface(this, savedInstanceState);
		if( OpDebug.LOG )
	    ...
	    orientationEventListener = new OrientationEventListener(this) {
			...
			public void onOrientationChanged(int orientation) {
				OpMainActivity.this.mainUI.onOrientationChanged(orientation);
			}
        };
		if( OpDebug.LOG )
        ...
        new Thread(new Runnable() {
            public void run() {
                textToSpeech = new TextToSpeech(OpMainActivity.this, new TextToSpeech.OnInitListener() {
        			@Override
	...
	private void setModeFromIntents(Bundle savedInstanceState) {
		...
		else if( OpTileService.TILE_ID.equals(action) ) {
			if( OpDebug.LOG )
				Log.d(TAG, "launching from quick settings tile for Open Camera: photo mode");
			applicationInterface.setVideoPref(false);
		}
		else if( OpTileServiceVideo.TILE_ID.equals(action) ) {
			if( OpDebug.LOG )
				Log.d(TAG, "launching from quick settings tile for Open Camera: video mode");
			applicationInterface.setVideoPref(true);
		}
		else if( OpTileServiceFrontCamera.TILE_ID.equals(action) ) {
			if( OpDebug.LOG )
    ...
    protected void onResume() {
        ...
        // Note that we do it here rather than customising the theme's android:windowBackground, so this doesn't affect other views - in particular, the OpPreferenceFragment settings
	...
	private void setProgressSeekbarScaled(SeekBar seekBar, double min_value, double max_value, double value) {
		...
		double frac = OpMainActivity.seekbarScalingInverse(scaling);
    ...
    public void openSettings() {
		...
		OpPreferenceFragment fragment = new OpPreferenceFragment();
	...
	private OpPreferenceFragment getPreferenceFragment() {
        return (OpPreferenceFragment)getFragmentManager().findFragmentByTag("PREFERENCE_FRAGMENT");
    }
    
    @Override
    public void onBackPressed() {
        final OpPreferenceFragment fragment = getPreferenceFragment();
        if( screen_is_locked ) {
    ...
    public void setWindowFlagsForCamera() {
    	...
    	/*{
    		Intent intent = new Intent(this, OpWidgetProvider.class);
    		intent.setAction(AppWidgetManager.ACTION_APPWIDGET_UPDATE);
    		AppWidgetManager widgetManager = AppWidgetManager.getInstance(this);
    		ComponentName widgetComponent = new ComponentName(this, OpWidgetProvider.class);
    ...
    public void updateGalleryIcon() {
		...
		new AsyncTask<Void, Void, Bitmap>() {
			private static final String TAG = "OpMainActivity/AsyncTask";
		    ...
		    protected Bitmap doInBackground(Void... params) {
				...
				KeyguardManager keyguard_manager = (KeyguardManager)OpMainActivity.this.getSystemService(Context.KEYGUARD_SERVICE);
	...
	public static class MyFolderChooserDialog extends FolderChooserDialog {
		...
		public void onDismiss(DialogInterface dialog) {
			...
			// so we access the OpMainActivity via the fragment's getActivity().
			OpMainActivity main_activity = (OpMainActivity)this.getActivity();
		...
		alertDialog.setItems(items, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				if( which == clear_index ) {
				    ...
				    new AlertDialog.Builder(OpMainActivity.this)
					...
					if( which >= 0 && which < history.size() ) {
						...
						preview.showToast(null, getResources().getString(R.string.changed_save_location) + "\n" + save_folder_name);
						SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(OpMainActivity.this);
						SharedPreferences.Editor editor = sharedPreferences.edit();
						if( applicationInterface.getStorageUtils().isUsingSAF() )
	...
	private class MyGestureDetector extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            try {
        		...
        		final ViewConfiguration vc = ViewConfiguration.get(OpMainActivity.this);
		...
		}
		{
		    ...
		    focusSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
				...
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
					...
					double scaling = OpMainActivity.seekbarScaling(frac);
    ...
    public OpApplicationInterface getApplicationInterface() {
	...
	private void showPhotoVideoToast(boolean always_show) {
		...
		if( preview.isVideo() ) {
			...
			OpApplicationInterface.PhotoMode photo_mode = applicationInterface.getPhotoMode();
			if( photo_mode == OpApplicationInterface.PhotoMode.DRO ) {
				photo_mode_string = getResources().getString(R.string.photo_mode_dro);
			}
			else if( photo_mode == OpApplicationInterface.PhotoMode.HDR ) {
				photo_mode_string = getResources().getString(R.string.photo_mode_hdr);
			}
			else if( photo_mode == OpApplicationInterface.PhotoMode.ExpoBracketing ) {
		...
		if( ok ) {
			...
			.setOnDismissListener(new OnDismissListener() {
				public void onDismiss(DialogInterface dialog) {
					...
					ActivityCompat.requestPermissions(OpMainActivity.this, permissions_f, permission_code);
				}
			}).show();
		}
	}
