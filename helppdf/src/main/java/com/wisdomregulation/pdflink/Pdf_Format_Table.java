 ...
 */
public class Pdf_Format_Table {
    public SimpleTableCell[][] array;
    public SimpleTableCell[][] arrayvale;

    public Pdf_Format_Table() {

    }

    public Pdf_Format_Table setArray(SimpleTableCell[][] array) {
        this.array = array;
        return this;
    }

    public Pdf_Format_Table setArrayValue(SimpleTableCell[][] arrayvale) {
            ...
            for (int i = 0; i < array.length; i++) {
                ...
                for (int j = 0; j < array[i].length; j++) {
                    ...
                    if (array[i][j].child == null) {
                        ...
                        PdfPCell cell = new PdfPCell(new Pdf_Format_Table().setArray(array[i][j].child).setArrayValue(array[i][j].childvalue).build());
                        if(array[i][j].rowspan!=-1){
                            cell.setRowspan(array[i][j].rowspan);
                        }
                        if(array[i][j].colspan!=-1){
